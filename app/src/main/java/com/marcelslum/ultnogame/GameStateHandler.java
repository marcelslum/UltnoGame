...
public class GameStateHandler{
    ...
    public static int previousMenuGoogleState;
    ...
    public static void makeTransitionBetweenStates(int previousState, int newState){
        ...
        if (previousState == GAME_STATE_INTRO){
                ...
                //  - GAME_STATE_MENU_RANKING
                // - GAME_STATE_MENU_GOOGLE

                MenuHandler.menuInicial.blockAndClearDisplay();
                MessagesHandler.messageMaxScoreTotal.clearDisplay();


                if (newState != GAME_STATE_OPCOES && newState != GAME_STATE_MENU_JOGAR && newState != GAME_STATE_MENU_GOOGLE){
            ...
            } if (newState == GAME_STATE_MENU_GOOGLE) {

            } if (newState == GAME_STATE_SELECAO_TUTORIAL) {
                Game.tittle.clearDisplay();
            }  else {
                Game.tittle.clearDisplay();
            }

        } else if (previousState == GAME_STATE_MENU_GOOGLE) {
            // vai para MENU_PRINCIPAL, MENU_JOGAR, MENU_OPCOES
            ...
            if (newState == GAME_STATE_MOSTRAR_OBJETIVOS || newState == GAME_STATE_SELECAO_TUTORIAL){
                MessagesHandler.starForMessage.clearDisplay();
                MessagesHandler.messageConqueredStarsTotal.clearDisplay();
                MenuHandler.menuTutorialUnvisited.blockAndClearDisplay();
            }
            ...
            Game.levelGoalsPanel.clearDisplay();
            ...
            // vai para PAUSE_OPCOES, PAUSE_OBJETIVO, JOGAR, SELECAO_LEVEL, INTERSTITIAL
            ...
            if (newState == GAME_STATE_PAUSE_OBJETIVO || newState == GAME_STATE_INTERSTITIAL || newState == GAME_STATE_SELECAO_LEVEL){
                MessagesHandler.messageInGame.clearDisplay();
            }
            ...
            MenuHandler.menuPauseOpcoes.blockAndClearDisplay();
    ...
    public static void setGameState(int newState){
        ...
        if (newState == GAME_STATE_INTERSTITIAL){
            ...
            if (GoogleAPI.connecting || GoogleAPI.disconnecting) {
                GoogleAPI.connecting = false;
                GoogleAPI.disconnecting = false;
            } else {
            ...
            if (Game.mainActivity.isSignedIn()) {
                MenuHandler.menuGoogleGeral.getMenuOptionByName("google").setText(Game.getContext().getResources().getString(R.string.deslogarGoogle));
            } else {
            ...
            */
            ...
            int totalStars = 0;
            for (int i = 0; i < Level.levelGoalsObject.levelGoals.size(); i++){
                if (Level.levelGoalsObject.levelGoals.get(i).achieved){
                    totalStars += Level.levelGoalsObject.levelGoals.get(i).numberOfStars;
                }
            }
            ...
            if (ScoreHandler.scorePanel.value > 0) {
                ...
                SaveGame.setLevelPoints(SaveGame.saveGame.currentLevelNumber, points);
            ...
            Level.levelGoalsObject.setFinish(TimeHandler.stopTimeOfLevelPlay());
            SaveGame.addLevelPlayed();
