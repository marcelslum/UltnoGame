...
public class Ball extends Circle{
    ...
    public float initial_dvx_padrao;
    public float initial_dvy_padrao;
    ...
    public void initializeVelocityAngleTimeRecord() {
        ...
        float angle = (float) Math.toDegrees(Math.atan2(Math.abs(dvy), Math.abs(dvx)));
        if ((angle - minAngle) > ((maxAngle - minAngle)/ 2f)){
            markMediumHighAngle();
        } else {
            markMediumLowAngle();
        }
        ...
        if ((lenOfInitialVelocity - minLen) > ((maxLen - minLen)/ 2f)){
            markMediumHighVelocity();
        } else {
            markMediumLowVelocity();
        }
    ...
    public void updateBaseVelocity(int newVelocity) {
        ...
        //Log.e(TAG, "initial_dvx_padrao " + initial_dvx_padrao);
        //Log.e(TAG, "initial_dvx_padrao " + initial_dvx_padrao);

        initialDVX = initial_dvx_padrao * ((float) newVelocity / 100);
        initialDVY = initial_dvy_padrao * ((float) newVelocity / 100);
    ...
    public void markMinVelocity() {
        ...
        if (!Training.training) {
            if (initTempoVelocidadeMinima == -1) {
                initTempoVelocidadeMinima = TimeHandler.timeOfLevelPlay;
            } else {
    ...
    public void markMaxVelocity() {
        ...
        if (!Training.training) {
            if (initTempoVelocidadeMaxima == -1) {
                initTempoVelocidadeMaxima = TimeHandler.timeOfLevelPlay;
            } else {
                tempoVelocidadeMaxima += (TimeHandler.timeOfLevelPlay - initTempoVelocidadeMaxima);
                initTempoVelocidadeMaxima = TimeHandler.timeOfLevelPlay;
            }
...
/*
        ...
        */
        ...
        for (int i = 0; i < quantityOfClones; i++) {
            ...
            // atualiza o dvx e dvy apenas para calcular os dados em initializeVelocityAngleTimeRecord()
            ball.dvx = explodeVelocityX;
            ball.dvy = explodeVelocityY;

            ball.initializeVelocityAngleTimeRecord();
    ...
    public void freeBall() {
        isFree = true;
        initializeVelocityAngleTimeRecord();
